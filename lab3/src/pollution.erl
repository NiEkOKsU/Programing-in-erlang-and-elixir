...
add_station(Name, {N, W}, M) when is_number(N), is_number(W), is_list(Name) ->
  ...
  if
    length(Filtred_name) > 0 -> M;
    length(Filtred_coordinates) > 0 -> M;
    true -> M#{{Name, {N, W}} => []}
  end.

add_value(Name, Data, Type, Value, M) when is_list(Name), is_list(Type), is_number(Value) ->
  Tab = name_filter(Name, maps:keys(M)),
  if
    Tab == [] -> M;
    true -> [{Key, _} | _] = Tab,
      #{Key := Val} = M,
...
get_one_value(Name, Data, Type, M) when is_list(Name), is_list(Type) ->
  ...
  if
    ...
    true ->   [{Key, _} | _] = Tab,
      #{Key := Val} = M,
      FiltredVal = [{A,B,C} || {{A, _},B,C} <- Val, A == Data, B == Type],
      if
        FiltredVal == [] -> brak;
        true ->  [{_,_,ToRet} | _] = FiltredVal, ToRet
      end
  end;
...
daily_avg_per_station([H|T], M) ->
  #{H := Values} = M,
  if
    Values == [] -> daily_avg_per_station(T, M);
    true ->   Dates = [{D, {0,0,0}} || {{D, _}, _, _} <- Values],
...
get_daily_average_data_count(M) ->
  ...
  if
    Keys == []-> brak;
    true ->  SumOfAvgsPerStation = daily_avg_per_station(Keys, M),
